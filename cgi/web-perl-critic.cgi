#!perl
use Web::Simple 'Web::Perl::Critic::Controller';
{
   package Web::Perl::Critic::Controller;

   BEGIN { use Moose } # if this isn't in a BEGIN block we get warnings
   ...
   sub _build_main {
      ...
      use HTML::Tags;
      use CSS::Declare;
      my $main = join '', HTML::Tags::to_html_string(
         <html>, <head>,
   ...
   sub _build_critic {
      ...
      return Web::Perl::Critic->new( directory => $self->config->{directory} );
   }

   sub criticisms {
      return [ 200, [ 'Content-type', 'application/json' ], [ JSON::encode_json( $self->critic->criticisms )] ];
   }

   default_config ( directory => q{.} );
...
Web::Perl::Critic::Controller->run_if_script;
